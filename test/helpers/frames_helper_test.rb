...
class TurboClone::FramesHelperTest < ActionView::TestCase
  ...
  test "frame with a block and string" do
    article = Article.new id: 1, content: "not important"

    assert_dom_equal %[<turbo-frame id="articles"><p>hey</p></turbo-frame>], turbo_frame_tag("articles") { tag.p("hey") }
  end

  test "frame with a block and model" do
    article = Article.new id: 1, content: "not important"
